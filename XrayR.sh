...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/weongit/XrayR-onekey-weongit/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/weongit/XrayR-onekey-weongit/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/weongit/XrayR-onekey-weongit/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/weongit/XrayR-onekey-weongit ---
  ${green}0.${plain} 修改配置
————————————————
